...
FETCH_DATA_REGISTRY = {
    ...
    "btc_price_num": lambda: _fetch_crypto_num("bitcoin"),
    "eth_price_num": lambda: _fetch_crypto_num("ethereum"),
...
# Resolved fetch vars from THIS run (read by the condition evaluator and
# persisted as last_<var> for cross-run "changed" checks).
_last_resolved: dict = {}
...
def _fetch_crypto_num(coin: str) -> float:
    """Numeric price for condition thresholds ({{btc_price_num}} > 100000)."""
    def _do_fetch():
        result = api_client.get_crypto_price_num(coin)
        if result.get("success"):
            return result["value"]
        return 0.0
    return _cached_fetch(f"crypto_price_num_{coin}", _do_fetch)
...
def resolve_content(payload: dict) -> dict:
    ...
    # Expose this run's resolved values to the condition evaluator
    _last_resolved.clear()
    _last_resolved.update(resolved)
...
# ============================================================================
# Declarative Conditions (agent capability)
# ============================================================================

def _to_number(value) -> float:
    """Best-effort numeric coercion ('$100,123.50' -> 100123.5)."""
    try:
...
def _evaluate_conditions(when: list, resolved: dict, state: dict) -> Tuple[bool, str]:
    """Evaluate payload["when"] — list of {var, op, value}, AND-combined.

    Ops: > < >= <= == != contains changed.
    'changed' compares the resolved var against state["last_<var>"].

    Returns (all_met, reason_when_not_met).
    """
    for cond in when or []:
        var = str(cond.get("var", ""))
        op = str(cond.get("op", "")).strip()
        expected = cond.get("value")
        if not var:
            return False, "condition missing var"
        if op == "changed":
            actual = resolved.get(var)
            prev = state.get(f"last_{var}")
            if prev is not None and str(prev) == str(actual):
                return False, f"{var} unchanged since last run"
            continue
        actual = resolved.get(var)
        if actual is None:
            return False, f"{var} not resolved"
        if op in (">", "<", ">=", "<="):
            a, b = _to_number(actual), _to_number(expected)
            if a is None or b is None:
                return False, f"{var} or value not numeric"
        ...
        if not ok:
            return False, f"{var} {op} {expected} not met (was {actual})"
    return True, ""


def _persist_last_state(resolved: dict) -> None:
    """Auto-persist resolved fetch vars as last_<var> in platform state so
    the 'changed' op works on the NEXT run. Never raises."""
    if not resolved:
        return
    try:
        state = api_client.state_get()
        state.update({f"last_{k}": v for k, v in resolved.items()})
        api_client.state_set(state)
    except Exception as e:
...
def execute_task(job: dict) -> dict:
    ...
    try:
        ...
        # Step 1.5: Declarative conditions — skip the handler when not met.
        # payload["when"] = [{"var": "btc_price_num", "op": ">", "value": 100000},
        #                    {"var": "views", "op": "changed"}]
        when = payload.get("when")
        if when:
            _state = api_client.state_get()
            ok, reason = _evaluate_conditions(when, _last_resolved, _state)
            _persist_last_state(_last_resolved)
            if not ok:
                logger.info(f"Condition skip: {reason}")
                return {"status": "success", "message": f"skipped: {reason}"}
        else:
            _persist_last_state(_last_resolved)
