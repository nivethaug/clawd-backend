...
def run_discord_bot_pipeline(
    ...
    try:
        ...
        # Log invite URL and setup instructions
        invite_url = token_info.get("invite_url", "")
        logger.info("=" * 60)
        ...
        if initial_env_vars:
            write_initial_environment_variables(str(Path(discord_path) / ".env"), initial_env_vars)
            logger.info(
                "Initial environment variables applied for discord project %s: %s",
                project_id,
                    ...
                    if usage:
                        ...
                        if _uid:
                            record_from_token_usage_json(
                                ...
                                project_id=project_id,
                                description=f"Discord bot create: {project_name}",
                            )
                            logger.info(f"✅ Token usage recorded for discord create")
