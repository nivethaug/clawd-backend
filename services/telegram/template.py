"""
Telegram Bot Template Copier
Copies the telegram-bot-template to project directory.
"""
import shutil
...
TEMPLATE_SOURCE = Path(__file__).parent.parent.parent / "templates" / "telegram-bot-template"


def copy_telegram_template(project_path: str) -> Tuple[bool, str]:
    """
    Copy telegram bot template to project directory.
    ...
    Returns:
        ...
        - If success: (True, "Path to telegram/ directory")
    ...
    try:
        ...
        # Create target path
        target_path = Path(project_path) / "telegram"
        
        # Remove existing telegram directory if exists
        if target_path.exists():
            logger.warning(f"Removing existing telegram directory: {target_path}")
            shutil.rmtree(target_path)
        
        # Copy template
        logger.info(f"Copying telegram template from {TEMPLATE_SOURCE} to {target_path}")
        ...
        logger.info(f"✅ Telegram template copied successfully to {target_path}")
...
def verify_template_structure(template_path: str) -> Tuple[bool, list]:
    ...
    Args:
        template_path: Path to telegram/ directory
