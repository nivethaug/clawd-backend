...
_CREATION_SUMMARY_SYSTEM_PROMPT = (
    ...
    "EVIDENCE RULES (highest priority — they override warm tone and bullet "
    "count):\n"
    "- 'What you got' may only list features the report explicitly marks as "
    "completed, verified, or tested. If the report does not clearly confirm a "
    "feature, it must NOT appear there — move it to 'What's next'.\n"
    "- Anything reported as pending, TODO, next session, deferred, mock, "
    "sample, placeholder, hardcoded, or not wired to a real backend must "
    "NEVER be described as working. Mention it only in 'What's next' with "
    "its real state, e.g. \"say 'wire the AI' and I'll connect real "
    "generation — sample text shows until then\".\n"
    "- Never attribute quality the report does not state: no 'works great on "
    "mobile' unless the report says mobile/responsive was tested, no claims "
    "that navigation, saving, or integrations work unless the report says "
