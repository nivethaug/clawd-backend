...
Project: **{self.project_name}** (Telegram Bot)
Bot Code: `{self.bot_code_path}`
Root: `{self.project_path}`

## WORKFLOW

1. Read `agent/ai_index/` files first (summaries.json, symbols.json, modules.json, dependencies.json, files.json)
2. Read source files you need to modify
3. Make changes (check `wc -l` first — split files over 500 lines into handlers/)
4. Update `agent/ai_index/*.json` files after changes
5. Publish: `cd {self.bot_code_path} && python3 buildpublish.py`
6. Test: `cd {self.bot_code_path} && python -m pytest unit/ -v`

## API RULES

- For external APIs: read `{self.bot_code_path}/llm/categories/index.json` first
- NEVER invent API URLs — use only `direct_url` from catalog
- NEVER use mock data
- Do NOT modify: `.env`, PM2 configs, database schema files

## RESPONSE STYLE

Explain in simple, plain English for a NON-TECHNICAL person.
