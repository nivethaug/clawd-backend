...
def build_initial_integrations_prompt_block(items: List[Dict[str, str]]) -> str:
    ...
    lines.extend([
        "",
        "Integration rule:",
        "- Before using a provided env integration, fetch the docs with `curl -L --max-time 20 <DOCS_URL>`.",
        "- Inspect base URL, endpoint path, auth method, headers, params, and response shape.",
        "- Run a one-off API test using the env var before writing source code.",
        "- If docs cannot be fetched, endpoint/auth is unclear, or the API test fails, create an isolated mock/fallback method.",
        "- Add a code comment above that mock explaining the exact reason, log a clear runtime warning/error when it is used, and keep it easy to replace.",
        "- Surface a concise user-facing notice where appropriate so the generated app/bot does not pretend the real integration is active.",
    ])
    return "\n".join(lines)
