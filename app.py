...
# not share that disk. Resolution order for /images/chat/*:
#   1. local file (worker itself + single-box deployments — zero config)
#   2. proxy from an existing worker env: DREAMPILOT_WORKER_API_URL or
#      WORKER_VPS_URL (bare base URLs, already configured on the fronting
#      box for admin cleanup / run proxying). CHAT_IMAGE_UPSTREAM_URL
#      overrides both if a dedicated target is ever needed.
CHAT_IMAGE_UPSTREAM = (
    os.getenv("CHAT_IMAGE_UPSTREAM_URL")
    or os.getenv("DREAMPILOT_WORKER_API_URL")
    or os.getenv("WORKER_VPS_URL")
    or ""
).rstrip("/")
...
5b. WEBSITE PAGE-INTEGRATION CHOICE (for website ideas with 3+ pages): do NOT ask early. Emit the TWO picker items as "required_env" IN THE SAME FINAL TURN as the brief (after propose_brief is accepted) — the platform renders them inside the final create section: (a) {"key": "REAL_DATA_PAGES", "type": "pages", "options": [<the app's pages> + "Login/Signup"], "question": "Pick up to 2 pages for real saved data (suggestions: <your two best-fit pages>)"} — a checkbox list, max 2, your suggestions named in the question; ALWAYS include "Login/Signup" as an option (if chosen, it becomes one of the two real-data pages via the template's existing auth service — no custom auth backend). (b) {"key": "SAVE_TARGET", "type": "choice", "options": [<primary saveable things, e.g. "Library drafts", "Settings preferences", ...>], "question": "Which one thing should the app SAVE first?"} — single pick; that feature gets the one write endpoint. Both answers come back as USER UI CHOICES in the next turn's context and go verbatim into the brief's Backend section. Your reply with the pickers should say the selections happen in the final setup section. If the user is in a hurry ("just build it"), skip the pickers, default to your suggestions + JSON-file persistence, and say so in one line.
6. AGENT DELIVERY CHANNELS: when the idea involves recurring output (daily reports, alerts, digests, keyword lists, notifications, monitoring), ask which channel(s) the user wants — list the four options (Telegram / Discord / Email / Webhook-API) and that ANY COMBINATION works, e.g. "email me daily AND ping Discord when something important is found". CRITICAL TIMING: the channel-selection question itself MUST go out with required_tokens null and required_env null — emitting a channel credential with the question pops a masked input before the user has chosen anything. Only in the turn AFTER the user names a channel do you collect what it needs, exactly like bot tokens:
   ...
   - Email → ask for their email address in chat, emitting {"key": "EMAIL_TO", ...} in "required_env". No credentials needed — DreamAgent sends from its own mail server.
...
"required_tokens" rules: list ONLY keys the user explicitly confirmed this project needs, that are NOT in the connected env keys, and are NOT the project type's required bot token. Allowed keys: OPENAI_API_KEY, ANTHROPIC_API_KEY, GEMINI_API_KEY, OPENROUTER_API_KEY, SERPER_API_KEY, RESEND_API_KEY, COINGECKO_API_KEY, STRIPE_SECRET_KEY, TELEGRAM_BOT_TOKEN and DISCORD_WEBHOOK_URL (agent delivery-channel credentials — label them "Telegram Bot Token" / "Discord Webhook URL"). NEVER invent other key names — for an API without a listed key, keep it as a brief note instead ("add <X> later in project settings"). NEVER produce a brief while required_tokens are still unattached — the platform blocks create until they're verified.
...
async def create_project_assistant(
    ...
    try:
        ...
        # page inference keep PROMPT_ASSISTANT_MODEL). Default: qwen3.7-flash —
        # probe 2026-09-26: 3/3 tool calls, cheapest per real turn. Gemini
        # 2.5-flash-lite was fastest (0.43s tool latency) but rejected live
        # 2026-09-26: narrated the env popup instead of filling required_env
        # (format-discipline miss; glm-4.7-flash died the same way).
