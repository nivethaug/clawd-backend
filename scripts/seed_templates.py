...
GALLERY: List[Dict[str, Any]] = [
    ...
    {"name": "Telegram Quiz Bot", "type_id": 2, "bot_token_env": "TELEGRAM_BOT_TOKEN_4", "description": "Create 'QuizMaster,' a Telegram quiz bot that makes trivia nights feel like a game show. The /quiz command launches a multiple-choice question with a countdown timer and 4 inline keyboard buttons. Players tap their answer instantly — correct responses show a green checkmark with points awarded, wrong answers show the correct option in red. A live leaderboard updates after every question showing rankings with medal emojis. The /create command lets admins build custom quiz packs with questions, answers, and point values. Categories include Science, History, Movies, Tech, and Sports with difficulty levels (Easy, Medium, Hard). A streak multiplier rewards consecutive correct answers with bonus points. The quiz end screen shows a podium with the top 3 players and their scores. Inspired by Kahoot's energy and HQ Trivia's tension."},
    ...
    {"name": "Discord AI Chat Bot", "type_id": 3, "bot_token_env": "DISCORD_BOT_TOKEN_4", "description": "Build 'Sage,' a Discord AI chat bot that brings intelligent conversation to any server. The /ask slash command streams thoughtful responses with typing indicators that build anticipation. The bot creates dedicated thread replies for long answers, keeping channels clean. A context window remembers the last 10 messages per channel so follow-up questions feel natural. The /summarize command reads the last 50 messages in any channel and produces a concise bullet-point summary. /translate handles 20+ languages with source auto-detection. A configurable personality system lets admins set a custom system prompt — 'helpful tutor,' 'sarcastic critic,' or 'friendly companion.' Rate limiting prevents abuse with per-user cooldowns. Every response uses clean embeds with the bot's branded color scheme. Inspired by Claude's clarity and Midjourney's thread-based UX."},
